// Astro pages are always root-relative (no /pages/ URL segment), so image paths need no prefixing.
const resolveAssetPath = (path) => (path.startsWith("http") ? path : "/" + path);
...
function renderPageAlbum() {
	...
	link.innerHTML = `<a class="button secondary" href="/gallery">View all photos</a>`;
